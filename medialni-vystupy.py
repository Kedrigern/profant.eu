...
def article_clean(article):
    ...
    try:
        ...
        if e.args[0] == "tags":
            ...
            print(
                "KeyError: missing {0} key in {1} in func article clean".format(
                    e, title[0]
                )
            )
...
def result_output():
    ...
    with open(Config.resultPath, "w", newline="\n") as csvfile:
        ...
        for a in Result.articles:
            try:
                spamwriter.writerow(
                    [a["date"], a["title"], a["author"], a["tags"], a["url"]]
                )
            except KeyError as e:
                print("KeyError: ", a, e)
                sys.exit()
...
def process_article(filepath, prefix):
    ...
    try:
        ...
        if e.args[0] == "author":
            ...
            print(
                "KeyError: missing {0} key in {1} in the func process_article".format(
                    e, filepath
                )
            )
